...
sed -i "s/TAIGA_HOSTNAME/$TAIGA_HOSTNAME/g" /conf/taiga/conf.json.json

# Look to see if we should set the "eventsUrl"
if [ ! -z "$RABBIT_PORT_5672_TCP_ADDR" ]; then
  echo "Enabling Taiga Events"
  sed -i "s/eventsUrl\": null/eventsUrl\": \"ws:\/\/$TAIGA_HOSTNAME\/events\"/g" /conf/taiga/conf.json.json
...
if [ "$TAIGA_SSL_BY_REVERSE_PROXY" = "True" ]; then
  ...
  sed -i "s/http:\/\//https:\/\//g" /conf/taiga/conf.json.json
  sed -i "s/ws:\/\//wss:\/\//g" /conf/taiga/conf.json.json
elif [ "$TAIGA_SSL" = "True" ]; then
  echo "Enabling SSL support!"
  sed -i "s/http:\/\//https:\/\//g" /conf/taiga/conf.json.json
  sed -i "s/ws:\/\//wss:\/\//g" /conf/taiga/conf.json.json
  mv /etc/nginx/ssl.conf /etc/nginx/conf.d/default.conf
elif grep -q "wss://" "/conf/taiga/conf.json.json"; then
  echo "Disabling SSL support!"
  sed -i "s/https:\/\//http:\/\//g" /conf/taiga/conf.json.json
  sed -i "s/wss:\/\//ws:\/\//g" /conf/taiga/conf.json.json
fi
